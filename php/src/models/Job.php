...
class Job
{
    ...
    public function getLowonganGuestById($lowonganId)
    {
        $query = "SELECT *
            FROM lowongan l
            JOIN user u ON l.company_id = u.user_id
    ...
    public function editLowongan($lowonganId, $posisi, $deskripsi, $jenisPekerjaan, $jenisLokasi, $isOpen, $attachment, $attachmentCount): bool
    {
        $uploadDir = '/var/www/html/uploads/';
        $this->conn->beginTransaction();
        try {
            ...
            $this->conn->rollBack();
            error_log("Failed to edit lowongan: " . $e->getMessage());
            return false;
        }
    }
    ...
    public function deleteLowonganCompany(int $lowonganId): bool
    {
        ...
        try {
            ...
            $applicants = $this->getApplicantsByLowonganId($lowonganId);
